...
class EquipmentData(BaseModel):
    brand: str | None = Field(description="Manufacturer name (e.g., Kingrinder, Lelit, AVX)")
    model: str | None = Field(description="Exact model name (e.g., K6, Bianca, Hero Plus)")
    equipment_type: str | None = Field(description="Can only be 'grinder' or 'espresso_machine'")
    burr_size_mm: int | None = Field(description="For grinders: burr diameter in millimeters. For machines: null.")
    burr_type: str | None = Field(description="For grinders: 'Conical' or 'Flat'. For machines: null.")
    boiler_type: str | None = Field(description="For machines: 'Single', 'HX', or 'Dual Boiler'. For grinders: null.")
    key_features: list[str] = Field(description="Maximum 5 most important features in English.")
